...
extension UsageMenuCardView.Model {
        ...
        preferredCurrencyCode: String = "auto") -> ProviderCostSection?
    {
        ...
        if let section = Self.openCodePayAsYouGoCostSection(provider: provider, cost: cost) {
            return section
        }
    ...
    private static func openCodePayAsYouGoCostSection(
        provider: UsageProvider,
        cost: ProviderCostSnapshot) -> ProviderCostSection?
    {
        guard provider == .opencode, cost.limit <= 0 else { return nil }
